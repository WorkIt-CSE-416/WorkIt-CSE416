...
**v1 does not use the database.** The API's schema is not finished, and waiting
on it would block the whole pipeline behind someone else's ticket. Instead the
store is a narrow interface, and v1 implements it over local files.

```
Store (protocol)
├── JsonStore      v1. Files under scraper/.scraped/. No database, no credentials.
└── PostgresStore  v2. The tables below, over the shared SQLAlchemy engine.
```

This is not a throwaway. The interface is the same contract in both cases —
read source state, compare versions, commit one board atomically, return the
outcome — so lifecycle, closure, reopening, content hashing, and conditional
requests are all exercised in v1 and none of them get rewritten in v2. What
changes is where the bytes land.

**One file per board is what makes it atomic.** Each source owns
`jobs/<source_key>.jsonl` and its entry in `state.json`; committing a board
means writing a temp file and `os.replace()`-ing it, which is atomic on POSIX
and Windows alike. A crash mid-write leaves the previous version intact, which
is the same guarantee the database transaction provides and the same one the
lifecycle rules depend on.

```
scraper/.scraped/                 gitignored, always
  state.json                      registry, checkpoints, versions, health
  jobs/<source_key>.jsonl         normalized postings, one per line
  payloads/<source_key>.jsonl     replay material
  runs.jsonl                      run and per-board outcomes, appended
  exports/jobs-<timestamp>.csv    flattened, for humans
```

**JSONL is the canonical format, CSV is a view.** Postings have nested
locations and compensation tiers; flattening them into cells loses data, and
the rule against manufacturing values applies to our own output too. `export`
writes a CSV — stdlib `csv`, opens in Excel — with the nested fields
JSON-encoded in single columns and a note that it is lossy. XLSX buys nothing
here and costs a dependency.

**What v1 cannot verify.** Multi-writer concurrency, real row locking, and
rollback across related tables are properties of Postgres, not of files. The
version-comparison logic is exercised, but two workers racing is not. Those
checks move to v2 — see the verification suite's Gate D, which is explicitly
...
The planned CLI has `discover`, `poll`, `reprocess`, and `export` commands.
Invoking `workit-scrape` without a subcommand defaults to `poll`. `poll`
selects its store with `--store json|postgres`, defaulting to `json` while v1
is current; `export` reads the JSON store and writes a flattened CSV. Planned npm
...
Milestones 1-6 need no database. That ordering is deliberate: the API's schema
is owned by another ticket, and nothing here should wait on it.
...
3. Define the `Store` protocol and implement `JsonStore` over
   `scraper/.scraped/` — lifecycle, closure, reopening, and the version check,
   committed one board at a time by atomic file replacement. Add `export` for
   CSV. **The pipeline is end-to-end usable here, with no database.**
4. Add Lever, Ashby, and SmartRecruiters against their provider contracts.
5. Add conservative conditional requests, pending-work recovery, and replay;
   pass the cache/lifecycle interaction tests.
6. Import pinned candidate data and run the pilot against `JsonStore`, with
   metrics. Sweeping stays disabled.
7. After KAN-93 and the KAN-50 dependency land, add the five ingestion tables
   as SQLAlchemy models with their constraints, generate and hand-review one
   Alembic revision, and implement `PostgresStore` against the same protocol.
   Autogenerated revisions are a draft: review the emitted DDL before
   committing it, because Alembic does not reliably detect constraint or index
   changes.
8. Adapt and pass lifecycle, rollback, and concurrent-writer tests against an
   isolated Postgres database before enabling sweeps.
9. Expand under the rollout criterion. Scheduler deployment and UI wiring
   remain follow-ups.
